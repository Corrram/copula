import Copula.Rank.Region.MeanVariance.Paper.Definitions
import Copula.Rank.Region.MeanVariance.Support.RankMoments
import Copula.Rank.Region.MeanVariance.Support.Mixture
...
theorem moment_representation (C : Copula 2) :
  ...
  ⟨C.spearmanRho_eq_one_sub, ProbabilityTheory.Copula.RankRegion.MeanVariance.Support.footrule_eq_abs_moment C⟩
...
theorem quadratic_upper_bound (C : Copula 2) :
  ...
  · simp only [probReal_univ, smul_eq_mul, one_mul] at hv
    have hm : C.spearmanFootrule = 1 - 3 * m := ProbabilityTheory.Copula.RankRegion.MeanVariance.Support.footrule_eq_abs_moment C
...
theorem fixed_footrule_intermediate (C D : Copula 2) {p r : ℝ}
  ...
  have hc : Continuous (fun a : I => (D.mix C a).spearmanRho) := by
    ...
    fun_prop
  obtain ⟨a, ha⟩ := ProbabilityTheory.Copula.RankRegion.MeanVariance.Support.exists_unitInterval_eq hc

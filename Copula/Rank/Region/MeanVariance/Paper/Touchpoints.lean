import Copula.Rank.Region.MeanVariance.Paper.Moments
import Copula.Rank.Region.MeanVariance.Support.EqualBlocks
...
namespace ProbabilityTheory.Copula.RankRegion.MeanVariance
...
theorem quadratic_defect (C : Copula 2) :
  ...
  have hm : (1 - C.spearmanFootrule) / 3 = m := by
    rw [ProbabilityTheory.Copula.RankRegion.MeanVariance.Support.footrule_eq_abs_moment]
    dsimp [m]
    ring
  ...
  · simp only [probReal_univ, smul_eq_mul, one_mul]
    rw [C.spearmanRho_eq_one_sub, ProbabilityTheory.Copula.RankRegion.MeanVariance.Support.footrule_eq_abs_moment]
    dsimp [m]
    ring
...
noncomputable def contactCopula (n : ℕ) : Copula 2 := ProbabilityTheory.Copula.RankRegion.MeanVariance.Support.equalBlocks n halfTurn
...
theorem contactCopula_coefficients (n : ℕ) :
  ...
  simp only [contactCopula, ProbabilityTheory.Copula.RankRegion.MeanVariance.Support.equalBlocks_footrule, ProbabilityTheory.Copula.RankRegion.MeanVariance.Support.equalBlocks_rho,
...
end ProbabilityTheory.Copula.RankRegion.MeanVariance

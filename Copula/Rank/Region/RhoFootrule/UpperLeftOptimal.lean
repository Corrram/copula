...
-/
import Copula.Rank.Region.RhoFootrule.UpperLeftMoments
...
noncomputable def dual (x : ℝ) : ℝ := potential S.N S.v S.w S.period_pos (x + S.w)

@[fun_prop] theorem continuous_dual : Continuous S.dual :=
  (continuous_potential (by exact_mod_cast S.N_pos) S.v_nonneg S.w_nonneg S.period_pos).comp (by fun_prop)

theorem dual_phase (k : ℕ) (i : Fin 4) (s : I) :
    S.dual (S.phase k i s) = pieceValue S.N S.v S.w i s := by
  unfold dual phase
  rw [sub_add_cancel]
  exact potential_translated_piecePoint (by exact_mod_cast S.N_pos) S.v_nonneg S.w_nonneg S.period_pos k i s
...
theorem path_contact (i : S.Index) (s : I) :
  ...
  rcases i with k | k | k
  · change S.dual (S.phase k 2 s) + S.dual (S.phase (k + 1) 0 s) = _

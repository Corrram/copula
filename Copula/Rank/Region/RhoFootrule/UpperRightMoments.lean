...
-/
import Copula.Rank.Region.RhoFootrule.UpperRightMarginal
...
theorem path_delta (i : S.Index) (s : I) :
  ...
  rcases i with k | k | k <;> dsimp [path, point, piecePoint, distance, period] <;>
    push_cast <;> ring
...
theorem integral_abs_distance :
    (∫ x, |(x 0 : ℝ) - x 1| ∂S.copula.toMeasure) =
      S.w + S.N * S.v + S.N * (S.N + 1) * S.v ^ 2 := by
  ...
  nlinarith only [congrArg (fun x : ℝ => x * (S.w + S.N * S.v)) S.normalized]
...
theorem integral_sq_distance :
  ...
  rw [h, hi]
  nlinarith only [congrArg (fun x : ℝ => x * (S.w + S.N * S.v) ^ 2) S.normalized]

theorem footrule :
    S.copula.spearmanFootrule = 1 - 3 * (S.w + S.N * S.v + S.N * (S.N + 1) * S.v ^ 2) := by
  rw [footrule_eq_abs_moment, S.integral_abs_distance]

theorem rho :
    S.copula.spearmanRho = 1 - 6 * ((S.w + S.N * S.v) ^ 2 +
      2 * S.N * (S.N + 1) * (S.w + S.N * S.v) * S.v ^ 2 +
      2 / 3 * S.N * (S.N + 1) * S.v ^ 3) := by

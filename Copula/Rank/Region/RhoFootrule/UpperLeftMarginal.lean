...
-/
import Copula.Rank.Region.RhoFootrule.UpperLeft
...
theorem path_density_sum (F : ℕ → Fin 4 → ℝ) :
    (∑ k ∈ Finset.range S.N, S.w * (F k 2 + F (k + 1) 0)) +
      (∑ k ∈ Finset.range S.N, (S.N - (k : ℝ)) * S.v * (F k 1 + F k 3)) +
      (∑ k ∈ Finset.range S.N, ((k : ℝ) + 1) * S.v * (F k 3 + F (k + 1) 1)) =
    (∑ k ∈ Finset.range S.N, (S.N * S.v * F k 1 + S.w * F k 2 +
      (S.N + 1) * S.v * F k 3 + S.w * F (k + 1) 0)) + S.N * S.v * F S.N 1 := by
...
theorem integral_four (f : ℝ → ℝ) (hf : Continuous f) (k : ℕ) :
    S.N * S.v * (∫ s : I, f (S.phase k 1 s)) +
      ...
      S.w * (∫ s : I, f (S.phase (k + 1) 0 s)) =
    ∫ x in S.phase k 1 0..S.phase (k + 1) 1 0, f x := by
  change S.width 1 * _ + S.width 2 * _ + S.width 3 * _ + S.width 0 * _ = _
  rw [S.integral_piece, S.integral_piece, S.integral_piece, S.integral_piece,
    (S.phase_adjacent k).2.1, (S.phase_adjacent k).2.2.1,
    (S.phase_adjacent k).2.2.2, (S.phase_adjacent (k + 1)).1]
...
theorem integral_periods (f : ℝ → ℝ) (hf : Continuous f) (m : ℕ) :
    (∑ k ∈ Finset.range m, ∫ x in S.phase k 1 0..S.phase (k + 1) 1 0, f x) =
      ∫ x in 0..S.phase m 1 0, f x := by
  induction m with
...
theorem phase_density_integral (f : ℝ → ℝ) (hf : Continuous f) :
    (∑ k ∈ Finset.range S.N, S.w * ((∫ s : I, f (S.phase k 2 s)) +
      ∫ s : I, f (S.phase (k + 1) 0 s))) +
  ...
  change _ + S.width 1 * _ = _
  rw [S.integral_periods f hf, S.integral_piece,
    intervalIntegral.integral_add_adjacent_intervals (hf.intervalIntegrable _ _) (hf.intervalIntegrable _ _)]
  have he : S.phase S.N 1 1 = 1 := by
...
theorem marginal_integral (j : Fin 2) (f : I → ℝ) (hf : Continuous f) :
  ...
  rw [Fin.sum_univ_eq_sum_range (fun k => S.w *
    ((∫ s : I, extend f (S.phase k 2 s)) + ∫ s : I, extend f (S.phase (k + 1) 0 s))),

...
theorem gammaPDF_mul_exp_neg {a t : ℝ} (ht : 0 ≤ t) (x : ℝ) :
...
theorem lintegral_exp_neg_gammaMeasure {a t : ℝ} (ha : 0 < a) (ht : 0 ≤ t) :
  ...
  rw [gammaMeasure, lintegral_withDensity_eq_lintegral_mul _ (by fun_prop) (by fun_prop)]
  simp_rw [gammaPDF_mul_exp_neg ht]
  rw [lintegral_const_mul _ (by fun_prop), lintegral_gammaPDF_eq_one ha (by positivity),
    mul_one]

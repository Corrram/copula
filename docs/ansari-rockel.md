...
| A04 | Gumbel–Hougaard | θ ≥ 1 | `exp(−((−log u)^θ+(−log v)^θ)^(1/θ))` | `gumbel`; full-square pointwise comonotonic endpoint |
...
| A12 | Nelsen 12 | θ ≥ 1 | `(1+((u^(−1)−1)^θ+(v^(−1)−1)^θ)^(1/θ))^(−1)` | `nelsen12`; `nelsen12_cdf_full`; full-square pointwise comonotonic endpoint |

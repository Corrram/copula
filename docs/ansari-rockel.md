...
| A04 | Gumbel–Hougaard | θ ≥ 1 | `exp(−((−log u)^θ+(−log v)^θ)^(1/θ))` | `gumbel`; full-square pointwise comonotonic endpoint; increasing lower-orthant order; exact lower/upper tail coefficients |
...
| A15 | Genest–Ghoudi (Nelsen 15) | θ ≥ 1 | `(1−((1−u^(1/θ))^θ+(1−v^(1/θ))^θ)^(1/θ))₊^θ` | `genestGhoudi`; `genestGhoudi_cdf_full`; exact lower/upper tail coefficients; non-CI, CDF/density-TP2 exclusions; CD iff θ=1 |
